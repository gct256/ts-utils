import { compareAll, compareFloat, compareValidatable } from "../modules/compare.ts";
...
/** a < b */
const isLT = (a: number, b: number): boolean => compareFloat(a, b) < 0;

/** a <= b */
const isLE = (a: number, b: number): boolean => compareFloat(a, b) <= 0;
...
/** Rectangle object. */
export class Rectangle extends BaseObject<RectangleData> implements RectangleData {
  /** @implements */
  ...
  private getPartForPoint({ x, y }: PointData): RectanglePart {
    const left = compareFloat(x, this.left);
    const right = compareFloat(x, this.right);
    const top = compareFloat(y, this.top);
    const bottom = compareFloat(y, this.bottom);
