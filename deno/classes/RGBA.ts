...
import { clamp } from "../modules/numbers.ts";
import { RGBAData } from "../types/RGBAData.ts";

import { BaseObject } from "./BaseObject.ts";

const getValue = (x: number): number =>
  Number.isFinite(x) ? clamp(Math.round(x), 0, 255) : x;
...
export class RGBA extends BaseObject<RGBAData> implements RGBAData {
  ...
  compare(other: RGBAData): number {
    ...
    return compareAll(
      compareValidatable(this, color),
      compareNumber(this.red, other.red),
      compareNumber(this.green, other.green),
      compareNumber(this.blue, other.blue),
      compareNumber(this.alpha, other.alpha),
    );
  }
